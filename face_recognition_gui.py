...
# Define augmentation pipeline for registration
...
def check_match(image):
    ...
    for e in encodes:
        distances = face_recognition.face_distance(EncodeListKnown, e)
        min_distance = np.min(distances)
        min_distance_index = np.argmin(distances)
        
        if min_distance <= tolerance:
            matched_name = StudentName[min_distance_index]
            base_name = matched_name.split('_aug_')[0]  # Extract original student ID
            matches.append(base_name)
            mark_attendance(base_name)
        else:
            matches.append("Unknown")
            logging.info(f"Face detected but not matched (distance: {min_distance})")

    if matches:
        if all(m == "Unknown" for m in matches):
            messagebox.showinfo("Results", "No registered students detected.")
        else:
            messagebox.showinfo("Results", f"Matches: {', '.join(set(matches))}")
    else:
        messagebox.showinfo("Results", "No matches found.")

def capture_student_image():
    name = simpledialog.askstring("Input", "Enter Student ID:")
    if not name or name in [n.split('_aug_')[0] for n in StudentName]:
        messagebox.showwarning("Warning", f"'{name}' already exists!" if name in [n.split('_aug_')[0] for n in StudentName] else "No name entered!")
        return
    ...
    while True:
        ...
        if key == ord('s'):
            save_path = os.path.join('D:/Project Agumentation/Students', f"{name}.jpg")
            cv2.imwrite(save_path, frame)
            frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            
            # Encode original and augmented versions
            encodings = []
            try:
                encodings.append(face_recognition.face_encodings(frame_rgb)[0])  # Original
            except IndexError:
                messagebox.showwarning("Warning", "No face detected in captured image!")
                break
            
            for i in range(3):  # 3 augmented versions
                aug_img = augmentations(image=frame_rgb)['image']
            ...
            if encodings:
                ...
                StudentName.extend([name] + [f"{name}_aug_{i}" for i in range(3)])
                with open('Encodefile.p', 'wb') as file:
                    pickle.dump((EncodeListKnown, StudentName), file)
                messagebox.showinfo("Success", f"Registered and encoded {name} with augmentations!")
            else:
                messagebox.showwarning("Warning", "No face detected in captured image!")
            break
        elif key == ord('q'):
...
title_label = tb.Label(root, text="Face Recognition & Attendance", font=("Arial", 30, "bold"), bootstyle="primary")
...
for text, command, style in buttons:
    tb.Button(root, text=text, command=command, bootstyle=f"{style}-outline", width=50, padding=15).place(x=570, y=y_position)
    y_position += 60

# Load and Place Logo in Bottom-Left
def place_logo():
    try:
        logo = Image.open("D:/Project Agumentation/logo/logo.png")  # Replace with your logo image path
